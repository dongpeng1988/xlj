...
public class Reporter : MonoBehaviour {
	...
	public void Initialize()
	{
        ...
        if (show && (Application.targetFrameRate == 46 || Application.loadedLevelName != "Battle"))
        {          
			doShow();
		}
	...
	void drawToolBar(){
		...
		{
			show = false;
            //SSJJ.Manager.EventManager.Instance.RaiseEvent("CloseDebug", null);
	...
	void Update()
	{
		...
		if( string.IsNullOrEmpty( scenes[0] ))
			scenes[0] = Application.loadedLevelName ;

		float elapsed = Time.realtimeSinceStartup - lastUpdate ;
		fps = 1f / elapsed ;
		lastUpdate = Time.realtimeSinceStartup ;
		calculateStartIndex();
		if(!show && isGestureDone()&&(Application.targetFrameRate==46||Application.loadedLevelName!="Battle"))
		{
			doShow();
		}
